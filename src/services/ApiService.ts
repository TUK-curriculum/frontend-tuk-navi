...
// Debounce 함수
const debounce = <T extends (...args: any[]) => any>(func: T, wait: number): ((...args: Parameters<T>) => Promise<ReturnType<T>>) => {
...
export interface GenerateTimetablesResult {
...
class ApiService {
  ...
  async getSemesters(): Promise<string[]> {
    ...
    try {
      ...
      return [`${currentYear - 1}-2학기`, `${currentYear}-1학기`, `${currentYear}-2학기`];
    }
  }
   ...
   * 시간표 생성/업데이트
   */
  async saveTimetable(timetableData: { semester: string; courses: any[]; updatedAt?: string; isGenerated?: boolean }): Promise<BackendTimetable> {
    try {
      ...
      if (existingTimetable?.id) {
        ...
        response = await apiClient.put<ApiResponse<BackendTimetable>>(`/timetable/${existingTimetable.id}`, payload);
      } else {
        console.log("[ApiService] Creating new timetable");
        response = await apiClient.post<ApiResponse<BackendTimetable>>("/timetable", payload);
      }
  ...
  async removeCourseFromTimetable(semester: string, courseId: number): Promise<BackendTimetable> {
    ...
    try {
      ...
      const updatedCourses = currentTimetable.courses.filter((course: any) => course.id !== courseId);
  ...
  async updateCourseInTimetable(
    ...
    try {
      ...
      const updatedCourses = currentCourses.map((course: any) => (course.id === courseId ? { ...course, ...updates } : course));
  ...
  async generateTimetables(
    curriculumId: number,
    preferences: any,
    excludeGenerated: string[] = []
  ): Promise<GenerateTimetablesResult> {
    try {
  ...
  // ===== 졸업 관리 =====
  async getGraduationStatus(): Promise<
    BackendGraduationStatus & {
  ...
  > {
    ...
    try {
      ...
      return {
        ...
        isGraduationReady: (statusData.pass?.total?.passed && statusData.pass?.major?.passed && statusData.pass?.liberal?.passed) || false,
        thresholds: {
  ...
  async getDashboardSummary(): Promise<{
    ...
    try {
      const [graduationStatus, timetable, notes, notifications] = await Promise.allSettled([
